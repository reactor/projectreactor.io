...
public class Application {
	...
	public Application() throws IOException {
		context = server.newRouter(r -> r.file("/favicon.ico", contentPath.resolve("favicon.ico"))
		                  ...
		                  .get("/docs/{module}/api/snapshot/**", forwardTo(true, true))
		                  .get("/docs/{module}/api/release/**", forwardTo(false, true))
		                  .get("/docs/{module}/reference/snapshot/**", forwardTo(true, false))
	...
	private BiFunction<HttpServerRequest, HttpServerResponse, Publisher<Void>> forwardTo(boolean isSnapshot, boolean isJavadoc) {
		return (req, resp) -> {
			String name = req.param("module").toString();
			Module module = modules.get(name) ;
			String versionType = isSnapshot ? "snapshot" : "release";
			String version = module
					.getVersions()
					.stream()
					.filter(v -> v
							.endsWith(versionType.toUpperCase()))
							.findFirst()
							.orElseThrow(() -> new IllegalStateException("No known version for module " + module));
			int offset = isJavadoc ? 12 : 18;
			String file = req.uri().substring(offset + versionType.length() + name.length());
			if (file.isEmpty()) {
				file = isJavadoc ? "index.html" : "docs/index.html";
			}
			String suffix = isJavadoc ? "-javadoc.jar" : ".zip";
			String url = "https://repo.spring.io/" + versionType + "/" + module.getGroupId().replace(".", "/") +
					"/" + module.getArtifactId() + "/" + version + "/" + module.getArtifactId() +
					"-" + version + suffix + "!/" + file;
			return client.get(url).then(r -> resp.send(r.receive().retain()));
		};
	}
